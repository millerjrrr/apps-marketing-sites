...
import TakeTheTestButton from "../AppSpecificComponents/WhoWhom/TakeTheTestButton";
...
const OpenBanner = () => {
  ...
  const appSite = getSiteContent().appMarketingSite;

  return (
    <div className="flex min-h-[700px] w-full flex-1 items-center justify-center">
      <div className="inner-container opening-banner overflow-x-hidden md:overflow-x-visible">
        <div className="inline-block flex w-full animate-pulseScale flex-col items-center justify-center text-center">
          ...
          {appSite && <BadgeContainer />}
          {siteKey === "who-whom" && <TakeTheTestButton />}
        </div>
        {appSite && (
